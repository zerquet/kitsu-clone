...
import { Component, ElementRef, inject, Input, OnInit, Renderer2, viewChild, ViewChild } from '@angular/core';
...
export class LibraryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userLibraryService.getLibrary(this.user.id).subscribe(res => this.userLibraryDataService.originalAnimeList$.next(res));
    this.authService.currentUser$.subscribe(user => {
      if(user !== null)
        this.canEdit$.next(user.id === this.user.id);
    });
  }
  ...
  openEditModal(entry: LibraryEntryWithAnimeInfo) {
    ...
    modal.componentInstance.initializeModalData({...entry}, this.canEdit$.value);
  }
